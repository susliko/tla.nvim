...
describe('tlc output parser should parse message:', function ()
  local function mk_msg(kind, lines) return { kind = kind, lines = lines } end
  local Message = {
    Coverage = mk_msg(
      p.MessageKind.Coverage,
      {'<Step1 line 56, col 1 to line 56, col 10 of module Module>: 5:35'}
    ),
    CoverageConstraint = mk_msg(
      p.MessageKind.CoverageConstraint,
      {'TODO'}
    ),
    CoverageEnd = mk_msg(
      p.MessageKind.CoverageEnd,
      {'End of statistics.'}
    ),
    CoverageInit = mk_msg(
    ...
    CovarageValueCost = mk_msg(
      p.MessageKind.CovarageValueCost,
      {'TODO'}
    ),
    Finished = mk_msg(
      p.MessageKind.Finished,
      {'Finished in 1000ms at (1970-00-00 00:00:00)'}
    ),
    InvariantViolated = mk_msg(
      p.MessageKind.InvariantViolated,
      {'Invariant NotSolved is violated.'}
    ),
    State = mk_msg(
      p.MessageKind.State,
      {'2: <Step1 line 56, col 16 to line 56, col 63 of module Module>',
       'contents = [j1 |-> 0, j2 |-> 5]',
       ''}
    ),
  }
  ...
  tag_ok('CoverageProperty', Message.CoverageProperty, '/path/to/file.tla', 'TypeOK\t/path/to/Module.tla\tnorm 34G1|\n')
